...
public class NewFriendsMsgAdapter extends ArrayAdapter<InviteMessage> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (msg != null) {
            ...
            holder.name.setText(msg.getFrom());// 屏蔽掉
//			holder.name.setText(msg.getNickName());
            ...
            if (msg.getAvator() != null && !msg.getAvator().equals("")) {
                // 设置用户头像
                Glide.with(context).load(msg.getAvator()).diskCacheStrategy(DiskCacheStrategy.ALL).into(holder.avator);
            }
...
//			public void run() {
        ...
        try {
            if (msg.getStatus() == InviteMesageStatus.BEINVITEED) {//同意好友请求
                EMClient.getInstance().contactManager().acceptInvitation(msg.getFromId());
                // 向服务器发送信息
                NetBaseRequest getUserRequest = RequsetFactory.creatBaseRequest(context, Constants.CHECK_FRIEND);
                getUserRequest.add("to_uid", msg.getFromId());
                CallServer.getRequestInstance().add(context, 0x01, getUserRequest, new HttpListenerCallback() {
                    @Override
                    public void onSucceed(int what, NetBaseBean netBaseBean) {
                        if (netBaseBean.isSuccess()) {
                            Toast.makeText(context, "已经同意", Toast.LENGTH_LONG).show();
                        } else {
                            Toast.makeText(context, netBaseBean.getMessage(), Toast.LENGTH_LONG).show();
                        }
                    }
