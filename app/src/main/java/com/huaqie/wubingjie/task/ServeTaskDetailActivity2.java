...
import com.huaqie.wubingjie.widgets.MyScrollview;
...
public class ServeTaskDetailActivity2 extends BaseActivity implements SwipeRefreshLayout.OnRefreshListener{
   ...
   /* private TextView tv_order_nickname,tv_order_school_isstudent,tv_order_phonenum;
    ...
    protected int getContentViewId() {
        return R.layout.activity_serve_task_detail2;
    }
    ...
    public static void goTo(Context context, String id, String type) {
        Intent intent = new Intent(context, ServeTaskDetailActivity2.class);
    ...
    protected void initView(Bundle savedInstanceState) {
        ...
        morePopupWindow = new MorePopupWindow(ServeTaskDetailActivity2.this);
        swipeRefreshLayout = (SwipeRefreshLayout) findViewById(swl);
        mScrollLayout = (MyScrollview) findViewById(R.id.scrollableLayout);
        //初始化 headview 数据
        ...
        CallServer.getRequestInstance().add(this, 0x03, request, new HttpListenerCallback() {
            ...
            public void onSucceed(int what, NetBaseBean netBaseBean) {
                if (netBaseBean.isSuccess()) {
                    T.showShort(ServeTaskDetailActivity2.this, "评论成功");
                    updateEditTextBodyVisible(View.GONE);
                    mEditText.getText().clear();
                    if (fragment != null) {
                        rg.check(R.id.rb_commment);
                        fragment.changeAdapter(1);
                    ...
                    T.showShort(ServeTaskDetailActivity2.this, netBaseBean.getMessage());
                }
            }

            @Override
            public void onFailed(int what, String url, Object tag, Exception exception, int responseCode, long networkMillis) {
                T.showShort(ServeTaskDetailActivity2.this, "评论失败");
            }
        }, true, true);
    ...
    private void callDetail() {
        ...
        }, false, !swipeRefreshLayout.isRefreshing());
    }
    ...
    private void updataUI() {
        try {
            ...
            if (taskItem.getPic_list().size() > 0) {
                ...
                for (int i = 0; i < taskItem.getPic_list().size(); i++) {
                    ...
                    imvpic.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            ImagePagerActivity.startImagePagerActivity(ServeTaskDetailActivity2.this,taskItem.getPic_list(), finalI,new ImagePagerActivity.ImageSize(width,width));
                        }
                    });
                }
            } else {
            ...
            ((ViewGroup)tv_address.getParent()).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    MapActivity.goTo(ServeTaskDetailActivity2.this, new MyLatlng(taskItem.getAddress(), taskItem.getLat(), taskItem.getLng()));
                }
            });
            tv_finish_time.setText("完成时间:  " + DateUtils.stringToString(taskItem.getLongFinish_time()+"",DateUtils.DB_DATA_FORMAT)+"之前");
    ...
    private void updataTitleBarUI() {
        ...
        morePopupWindow.getBtn_cancel().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!TextUtils.isEmpty(taskItem.getOrder_id())) {
                    CancelServeTaskActivity.goTo(ServeTaskDetailActivity2.this, taskItem.getOrder_id(), CancelServeTaskActivity.CANCEL, ServeTaskDetailActivity.class.getSimpleName());
                } else {
                    T.showShort(ServeTaskDetailActivity2.this, "order_id为 nulll");
                }
        ...
        morePopupWindow.getBtn_tousu().setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ComplainActivity.goTo(ServeTaskDetailActivity2.this, taskItem.getType(), taskItem.getServe_task_id());
                morePopupWindow.dismiss();
            }
        });
        ...
        switch (status) {
            ...
            case "3":
                ...
                btn2.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        FinishActivity.goTo(ServeTaskDetailActivity2.this, taskItem.getOrder_id(), taskItem.getFinish_msg(), ServeTaskDetailActivity.class.getSimpleName());
                    }
                });
                break;
            case "4":
                ...
                if (netBaseBean.isSuccess()) {
                    if (TextUtils.isEmpty(netBaseBean.getBody())) {
                        ...
                        if (grabOrder.getIs_pay() == 0) {
                            ...
                            //跳转支付
                            PayActivity.goTo(ServeTaskDetailActivity2.this, grabOrder.getPay_id(), taskItem.getType(), ServeTaskDetailActivity.class.getSimpleName());
                        }
                    }
                } else {
                    T.showShort(ServeTaskDetailActivity2.this, netBaseBean.getMessage());
