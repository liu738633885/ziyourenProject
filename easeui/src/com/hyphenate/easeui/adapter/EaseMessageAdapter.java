...
public class EaseMessageAdapter extends BaseAdapter {
	...
	Handler handler = new Handler() {
		private void refreshList() {
			...
			//语音到来自动播放语音
			if (getCount() != 0) {
				if (getItemViewType(getCount() - 1) == MESSAGE_TYPE_RECV_VOICE && !getItem(getCount() - 1).isListened() && !isVoicePlaying) {
					over(getCount() - 2);
				}
			}
		}
	...
	public void over(int position) {
		if (getCount() > position + 1) {
			//有多余的消息
			for (int j = position + 1; j < getCount(); j++) {
				if (getItemViewType(j) == MESSAGE_TYPE_RECV_VOICE) {//下面还有语音消息
					if (!getItem(j).isListened()) {
						//模拟点击操作
						voicePerformClick(j);
						return;
					} else {
	...
	private void voicePerformClick(int j) {
		//滑动------目前滑动距离过远可能会断
		listView.smoothScrollToPosition(j + 2);
		//点击
		EaseChatRowVoice ev = (EaseChatRowVoice) listView.getChildAt(j - listView.getFirstVisiblePosition());
		if (ev != null) {
			ev.clickme();
		} else {
			//listView.smoothScrollToPosition(j + 2);
