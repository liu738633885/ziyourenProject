...
public class IndexableStickyListView extends FrameLayout implements AdapterView.OnItemClickListener, AbsListView.OnScrollListener {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        //设置右侧选择器距离顶部的距离
        params.topMargin = IndexBar.dp2px(context, 70);
...
//            indexListView.mAdapter.setDatas(indexListView.mItems, indexListView.mHeaderEntities);
            indexListView.mAdapter.setDatas2(indexListView.mItems);
    ...
    public <T extends IndexEntity> void bindDatas(final List<T> items){
        mItems = new ArrayList<>();
        mItems.addAll(items);
        if (mAdapter == null) {
            return;
